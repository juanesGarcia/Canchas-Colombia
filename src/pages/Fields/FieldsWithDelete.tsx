...
export const FieldsWithDelete: React.FC = () => {
  ...
  const filteredFields = courts.filter((court) => {
    ...
    // Búsqueda incluye: nombre de cancha, ciudad Y nombre del propietario
    const matchesSearch = 
    ...
    const matchesType = selectedType === "all" || court.court_type?.toLowerCase().includes(selectedType);
    ...
    const matchesCity = selectedCity === "all" || court.city?.toLowerCase() === selectedCity.toLowerCase();
  ...
  const handleDelete = async (courtId: string) => {
    ...
    if (ok) {
      setCourts((prev) => prev.filter((court) => court.court_id !== courtId));
    } else {
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-8">
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Buscar por cancha, ciudad o propietario..."
              ...
              .map((field) => (
                <div key={field.court_id} className="relative">
                  <FieldCard 
                    field={field} 
                    onBook={() => {}}
                    // Si FieldCard no muestra owner_name, puedes pasarlo así:
                    // ownerName={field.owner_name}
                  />
                  
                  {/* Mostrar owner_name si existe */}
                  {field.owner_name && (
                    <div className="absolute bottom-3 left-3 bg-black/70 text-white text-xs px-2 py-1 rounded">
                      Propietario: {field.owner_name}
                    </div>
                  )}
